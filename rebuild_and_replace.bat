...
if "%ERRORLEVEL%"=="0" (
    echo.
    echo ⚠️  ВНИМАНИЕ: WarehouseApp.exe запущен!
    echo.
...
if not exist "dist\WarehouseApp_New.exe" (
    echo ❌ ОШИБКА: Файл dist\WarehouseApp_New.exe не найден!
    pause
    exit /b 1
)
echo ✓ Новый exe файл создан
...
if exist "WarehouseApp.exe" (
    ...
    echo ✓ Резервная копия создана: WarehouseApp_OLD_%timestamp%.exe
)
...
echo.
echo Что изменилось:
echo   ✓ Исправлена проблема с бесконечным открытием окон
echo   ✓ Добавлена возможность указывать объект/город/отдел на этикетках
echo   ✓ Исправлена проблема с выбором оборудования
echo.
